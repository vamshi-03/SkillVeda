...
export const changeRole = (data,userId) => (dispatch) => {
    ...
    fetch(`http://localhost:5000/users/Teachme/${userId}`, {
      method: "GET",
      headers: {
...
export const getCourse = (data,userId) => (dispatch) => {
  dispatch({ type: PRODUCT_REQUEST });
  fetch(`http://localhost:5000/courses/a/${userId}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`
...
export const addVideo=(data,courseId)=>(dispatch)=>{
  ...
  fetch(`http://localhost:5000/videos/add/${courseId}`,{
    method:"POST",
    headers:{
      "Content-Type":"application/json",
      Authorization:`Bearer ${token}`
...
export const getvideo=(page,limit,user)=>(dispatch)=>{
    dispatch({type:PRODUCT_REQUEST})
    axios.get(`http://localhost:5000/videos?page=${page}&limit=${limit}&user=${user}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
